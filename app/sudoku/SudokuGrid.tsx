...
export function SudokuCell({ index, ...props }: Props & { index: number }) {
  ...
  return <button className={classes} style={cellPlacement(row, col)} onClick={() => onSelect(index)} tabIndex={index === selected ? 0 : -1} aria-label={`Riadok ${row + 1}, stĺpec ${col + 1}, ${description}${conflicts.has(index) ? ", konflikt" : ""}`} data-index={index}>
