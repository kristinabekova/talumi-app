...
export default function SudokuGame({ size, resume, onBack, onChooseSize }: { size: SudokuSize; resume: boolean; onBack: () => void; onChooseSize: () => void }) {
  ...
  const enter = useCallback((number: number) => {
    ...
    commit(next => { next.values[selected] = number; next.notes[selected] = []; next.notes = next.notes.map((notes, index) => index !== selected && isPeer(index, selected, size) ? notes.filter(n => n !== number) : notes); }, "Číslo je na svojom mieste. Môžeš ho kedykoľvek upraviť."); tone("place");
  ...
  }, [enter, move, showComplete, paused, size]);
