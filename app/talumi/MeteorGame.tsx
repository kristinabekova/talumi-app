...
import { SoundToggle } from "./GameChrome";
...
export function MeteorGame({ onBack }: { onBack: () => void }) {
  ...
  return <main className={`app meteor-app ${reducedMotion ? "reduce-motion" : ""}`}>
    <header className="topbar"><button className="back" onClick={onBack} aria-label="Späť do Gaming zóny"><span>←</span> Gaming zóna</button><h1>Meteorický dážď</h1><div className="hud"><div className="hud-pill"><Icon>◷</Icon><strong>00:{String(timeLeft).padStart(2, "0")}</strong></div><div className="hud-pill"><Icon>◆</Icon><strong>Séria {streak}</strong></div><div className="hud-pill"><span className="coin">C</span><strong>{coins}</strong></div><SoundToggle on={soundOn} onClick={() => setSoundOn((v) => !v)} className="meteor-sound" /><button className="round-button" onClick={() => { if (gameState === "playing") setGameState("paused"); else if (gameState === "paused") { lastRef.current = performance.now(); setGameState("playing"); } }} aria-label={gameState === "paused" ? "Pokračovať" : "Pozastaviť"}>{gameState === "paused" ? "▶" : "Ⅱ"}</button></div></header>
