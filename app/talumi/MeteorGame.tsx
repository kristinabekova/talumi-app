...
export function MeteorGame({ onBack }: { onBack: () => void }) {
  ...
  return <main className={`app meteor-app ${reducedMotion ? "reduce-motion" : ""}`}>
    <div className="meteor-topbar"><GameTopBar title="Meteorický dážď" onBack={onBack} onPause={() => { if (gameState === "playing") setGameState("paused"); else if (gameState === "paused") { lastRef.current = performance.now(); setGameState("playing"); } }} paused={gameState === "paused"} sound={soundOn} onToggleSound={() => setSoundOn((v) => !v)} /></div>
